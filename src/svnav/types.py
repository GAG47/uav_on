...
class NavigationWaypoint:
    """
    Continuous waypoint selected by the navigator.

    It is not an executable UAV-ON action. The action must be generated by
    ActionAdapter / WaypointController.
    """
    episode_id: str
    step_id: int
    waypoint_id: str
    ...
    observe_position: Optional[Tuple[float, float, float]] = None
    source: str = "geometric_explore"
    target_type: str = "search_waypoint"
    target_id: Optional[str] = None
    region_id: Optional[str] = None
    arrive_radius: float = 5.0
    yaw_tolerance: float = 45.0
    reason: str = ""
    ...
    def to_log_dict(self) -> Dict[str, Any]:
        return {
            ...
            "observe_position": _json_safe(self.observe_position),
            "source": self.source,
            "target_type": self.target_type,
            "target_id": self.target_id,
            "region_id": self.region_id,
            "arrive_radius": float(self.arrive_radius),
            "yaw_tolerance": float(self.yaw_tolerance),
            "reason": self.reason,
...
class ControlCommand:
    """
    Low-level command generated by the waypoint controller.

    It still uses UAV-ON's executable action names, but the decision source is
    a continuous waypoint, not a semantic action.
    """
    action: str
    step_size: float
    source: str = "waypoint_controller"
    phase: str = "move_to_waypoint"
    reason: str = ""
...
class ControllerFeedback:
    """
    Execution feedback from waypoint controller to navigator/logging.
    """
    waypoint_id: Optional[str]
    reached: bool
    distance_to_waypoint: float
    yaw_to_waypoint: float
    observe_yaw_error: Optional[float] = None
    progress: float = 0.0
    no_progress_steps: int = 0
    rotate_loop_steps: int = 0
    ...
    def to_log_dict(self) -> Dict[str, Any]:
        return {
            ...
            "distance_to_waypoint": float(self.distance_to_waypoint),
            "yaw_to_waypoint": float(self.yaw_to_waypoint),
            "observe_yaw_error": self.observe_yaw_error,
            "progress": float(self.progress),
            "no_progress_steps": int(self.no_progress_steps),
            "rotate_loop_steps": int(self.rotate_loop_steps),
            "translation_safe": bool(self.translation_safe),
            "replan_needed": bool(self.replan_needed),
            "reason": self.reason,
            "debug_info": _json_safe(self.debug_info),
        }
