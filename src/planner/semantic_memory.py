...
class SemanticMemory:
    ...
    def get_best_memory_target(
        ...
        frontiers = self.get_semantic_frontiers(
            current_pose=current_pose,
    ...
    def build_frontier_candidate_mask(
        ...
        unvisited_mask = np.logical_not(self.visited)
    ...
    def get_observed_boundary_mask(self, observed_mask):
        ...
        for gx in range(self.grid_size):
            for gy in range(self.grid_size):
                ...
                for nx, ny in self.get_neighbors(gx, gy, eight_connected=False):
    ...
    def cluster_frontier_cells(
        ...
        for gx in range(self.grid_size):
            for gy in range(self.grid_size):
                ...
                cluster = []
                seed_semantic = float(self.semantic_value[gx, gy])
                ...
                while len(queue) > 0:
                    ...
                    for nx, ny in self.get_neighbors(cx, cy, eight_connected=True):
                        ...
                        semantic_diff = abs(float(self.semantic_value[nx, ny]) - seed_semantic)
    ...
    def build_frontier_info(self, frontier_id, cells, score_map, current_pose):
        ...
        for gx, gy in cells:
            ...
            semantic_values.append(float(self.semantic_value[gx, gy]))
            confidence_values.append(float(self.confidence[gx, gy]))
            safety_values.append(float(self.safety_value[gx, gy]))
            novelty_values.append(float(self.novelty_value[gx, gy]))
            visited_values.append(bool(self.visited[gx, gy]))
            observe_counts.append(int(self.observe_count[gx, gy]))
        ...
        distance_penalty = min(0.35, distance / max(self.map_size, 1e-6))
