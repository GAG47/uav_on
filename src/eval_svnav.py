...
from model_wrapper.ON_Air_SV import ONAirSV
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            for t in range(args.maxActions):
                ...
                print("[SVNav] get actions time:", time.time() - start1)
                ...
                for i in range(len(actions)):
                    print("[SVNav]", actions[i], ":", steps_size[i])

                env.makeActions(actions, steps_size, is_fixed)

                obs = env.get_obs()
                batch_state.update_from_env_output(
                    obs, user_prompts, actions, steps_size, is_fixed
                )
...
if __name__ == "__main__":
    ...
    modelWrapper = ONAirSV(fixed=fixed, batch_size=args.batchSize)
